...
public static class BudgetEndpoints
{
    ...
    {
        app.MapGet("/api/budget/stats", GetBudgetStatsAsync);
        app.MapPut("/api/budget/plan/{year:int}/{month:int}", UpsertBudgetPlanAsync);
    }

    private static async Task<IResult> GetBudgetStatsAsync(
        int? year,
        int? month,
        bool reusePlan,
        SolverDbContext db,
        HttpContext ctx)
    {
        ...
        var expenseAccounts = await db.Accounts
    ...
    private static async Task<IResult> UpsertBudgetPlanAsync(
        int year,
        int month,
        ...
        HttpContext ctx)
    {
        ...
        {
            var mode = row.InputMode?.Trim().ToLowerInvariant() == "amount" ? "amount" : "percent";
    ...
    private static async Task<(BudgetPlanMonth plan, (int year, int month)? copiedFrom)> GetOrCreatePlanMonthAsync(
        SolverDbContext db,
        Guid userId,
        int year,
        int month,
    ...
    private static async Task<(decimal averageIncome, decimal fixedExpensesTotal)> GetIncomeAndFixedExpensesAsync(
        SolverDbContext db,
        Guid userId,
        DateOnly monthStart)
    {
